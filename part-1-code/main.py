...
def create_augmented_dataloader(args, dataset):
    ...
    # Step 1: Get the original training dataset and tokenize it
    original_train = dataset["train"]
    original_tokenized = original_train.map(tokenize_function, batched=True)
    ...
    # Step 2: Create 5,000 transformed examples from the training set
    # Randomly sample 5k examples from training set
    augmented_examples = original_train.shuffle(seed=42).select(range(5000))
    
    # Apply our custom transformation
    transformed_examples = augmented_examples.map(custom_transform, load_from_cache_file=False)
    
    # Tokenize the transformed examples
    transformed_tokenized = transformed_examples.map(tokenize_function, batched=True)
    ...
    # Step 3: Concatenate original and transformed datasets
    from datasets import concatenate_datasets
    augmented_dataset = concatenate_datasets([original_tokenized, transformed_tokenized])
    
    # Step 4: Create and return the dataloader
...
if __name__ == "__main__":
    ...
    try:
        ...
        # Try loading again without split verification
